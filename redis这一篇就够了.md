...
- [redis集群](#redis集群)
    - [redis主备](#redis主备)
        - [命令](#命令-6)
        - [数据同步](#数据同步)
    - [sentinel 原理](#sentinel-原理)
            - [配置](#配置-1)
            - [原理](#原理)
            - [raft算法](#raft算法)
            - [启动](#启动)
            - [spring boot 引入　（默认应用lettuce客户端）](#spring-boot-引入　默认应用lettuce客户端)
        - [redis　cluster](#redis　cluster)
            - [配置](#配置-2)
            - [启动命令](#启动命令)
            - [其他命令](#其他命令)
            - [一致性hash算法](#一致性hash算法)
            - [redis hash 槽算法](#redis-hash-槽算法)
- [redis 常见开发问题解决方案](#redis-常见开发问题解决方案)
    - [redis客户端](#redis客户端)
    - [pipeline - 大数据量传输](#pipeline---大数据量传输)
        - [redus　conf 配置](#redus　conf-配置)
    - [数据一致性问题](#数据一致性问题)
        - [先操作数据库后删除redis缓存](#先操作数据库后删除redis缓存)
            - [解决方案](#解决方案)
        - [先删除缓存后更新数据库](#先删除缓存后更新数据库)
            - [解决方案](#解决方案-1)
    - [热点数据](#热点数据)
        - [客户端进行记录](#客户端进行记录)
        - [代理层记录](#代理层记录)
